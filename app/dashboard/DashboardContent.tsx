...
export default function DashboardContent({ favorites: initialFavorites, initialBuilds }: Props) {
  ...
  return (
    <div>
      {/* Tabs */}
      <div className="flex gap-2 mb-4 bg-[#080e1e]/80 p-1.5 rounded-2xl border border-purple-900/40" style={{ boxShadow: '0 0 20px rgba(139,92,246,0.06)' }}>
        <button
        ...
        >
          ⚔ My Builds {builds.length > 0 && (
            <span className={`ml-1.5 text-xs px-1.5 py-0.5 rounded-full ${activeTab === 'builds' ? 'bg-white/20' : 'bg-purple-900/50 text-purple-300'}`}>
              {builds.length}
            </span>
          )}
        </button>
        ...
        >
          ♥ Favourites {favorites.length > 0 && (
            <span className={`ml-1.5 text-xs px-1.5 py-0.5 rounded-full ${activeTab === 'favourites' ? 'bg-white/20' : 'bg-purple-900/50 text-purple-300'}`}>
              {favorites.length}
            </span>
          )}
        </button>
      ...
      {activeTab === 'builds' && (
        <div className="space-y-4">
          {/* Build form */}
          <div className="bg-[#0d1624] rounded-2xl border border-purple-900/40 p-4" style={{ boxShadow: '0 0 40px rgba(139,92,246,0.08)' }}>
            ...
            {favorites.length === 0 ? (
              <p className="text-xs text-gray-500 mb-3">
                ...
                <Link href="/" className="text-purple-400 hover:text-purple-300">Browse →</Link>
              </p>
            ) : (
              <select
              ...
              >
                ...
                {favorites.map(f => (
                  <option key={f.heroId} value={String(f.heroId)}>{f.heroName}</option>
                ))}
              </select>
            )}
                    ...
                    >
                      {item ? (
                        <>
                          ...
                          <span className="text-[9px] leading-tight text-gray-300 line-clamp-2 w-full">{item}</span>
                        </>
                      ) : (
                        <>
                          <div className="w-9 h-9 rounded-xl flex items-center justify-center text-purple-700/60" style={{ border: '1.5px dashed rgba(139,92,246,0.25)' }}>
                    ...
                    {item && (
                      ...
                      >
                        <svg width="7" height="7" viewBox="0 0 7 7" fill="none">
                          <path d="M1 1l5 5M6 1L1 6" stroke="#a78bfa" strokeWidth="1.4" strokeLinecap="round"/>
                        </svg>
                      </button>
                    )}
                  </div>
          ...
          {/* Saved builds list */}
          {builds.length === 0 ? (
            <div className="bg-[#0d1624] rounded-2xl border border-purple-900/40 p-8 text-center" style={{ boxShadow: '0 0 40px rgba(139,92,246,0.08)' }}>
            ...
            <div className="space-y-2">
              {builds.map(build => {
                const itemList = [build.item1, build.item2, build.item3, build.item4, build.item5, build.item6].filter(Boolean)
                return (
                  <div
                    key={build.id}
                  ...
                  >
                    {build.heroHead ? (
                      <img src={build.heroHead} alt={build.heroName} className="w-10 h-10 rounded-full object-cover flex-shrink-0 border border-[#1e2d55] bg-[#0f1a30]" />
                    ) : (
                      <div className="w-10 h-10 rounded-full bg-[#0f1a30] flex-shrink-0 border border-[#1e2d55]" />
                    )}
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-gray-100 mb-1">{build.heroName}</div>
                      {itemList.length > 0 ? (
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {itemList.map((item, i) => {
                            ...
                            return (
                              ...
                              >
                                {meta && <span className="text-[11px]">{meta.emoji}</span>}
      ...
      {/* Favourites tab */}
      {activeTab === 'favourites' && (
        <div className="bg-[#0d1624] rounded-2xl border border-purple-900/40 overflow-hidden" style={{ boxShadow: '0 0 40px rgba(139,92,246,0.08)' }}>
          <div className="flex items-center gap-2 px-4 py-3 border-b border-purple-900/25">
            ...
            <span className="ml-auto text-xs text-gray-500">{favorites.length} hero{favorites.length !== 1 ? 's' : ''}</span>
          </div>
          ...
          {favorites.length === 0 ? (
            ...
            <div className="p-3 space-y-1.5">
              {favorites.map(fav => (
                ...
                >
                  <Link href={`/?hero=${fav.heroId}`} className="flex items-center gap-3 flex-1 min-w-0">
                    {fav.heroHead ? (
                      <img
                        src={fav.heroHead}
                        alt={fav.heroName}
                    ...
                    <span className="text-sm font-semibold text-gray-100 truncate">{fav.heroName}</span>
                  </Link>
                  <HeartButton
                    heroId={fav.heroId}
